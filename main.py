...
### no using
### get txt dataset and dataloader
...
### using from main
### get lmdb dataset and dataloader
...
### no using
### the single get dataloader
def get_dataloader(synthetic_dataset, real_dataset, height, width, batch_size, workers,is_train, keep_ratio):
...
def main(args):
    ...
    if not args.evaluate:
        train_dataset, train_loader = get_data_lmdb(args.synthetic_train_data_dir, args.voc_type, args.max_len,
                                                    args.num_train,
                                                    args.height, args.width, args.batch_size, args.workers, True,
                                                    args.keep_ratio)

    test_dataset, test_loader = get_data_lmdb(args.test_data_dir, args.voc_type, args.max_len, args.num_test,
    ...
    # Create model
    model = SGTR(backbone_arch1=args.backbone_arch1,
		backbone_arch2=arg.backbone_arch2,voc_type=arg.voc_type ,max_len_labels=max_len,
    ...
    if args.evaluation_metric == 'accuracy':
        ...
        evaluation_metric = EditDistance 
        best_res = math.inf
    else:
    ...
    # Evaluator
    evaluator = Evaluator(model, evaluation_metric, args.cuda)
    ...
    loss_weights['loss_vr'] = 0.5
    loss_weights['loss_kl'] = 1
    if args.debug:
        args.print_freq = 1
    trainer = Trainer(model, evaluation_metric, args.logs_dir,
...
if __name__ == '__main__':
    #parse fron the config
    os.environ['CUDA_VISIBLE_DEVISES'] = '0,1,2,3'
